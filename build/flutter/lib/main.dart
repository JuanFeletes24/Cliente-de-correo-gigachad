...
import 'native_runtime_stub.dart' if (dart.library.ffi) 'native_runtime.dart'
    as nrt;
...
Future prepareApp() async {
  ...
  await setupDesktop(
    ...
    waitUntilReadyToShow: !(const bool.fromEnvironment("FLET_TEST") &&
  ...
  if (kIsWeb) {
    ...
    environmentVariables.putIfAbsent(
        "FLET_APP_STORAGE_DATA", () => appDataPath);
    environmentVariables.putIfAbsent(
        "FLET_APP_STORAGE_CACHE", () => appCachePath);
    environmentVariables.putIfAbsent(
        "FLET_APP_STORAGE_TEMP", () => appTempPath);
