...
* `cliente_correo_gigachad` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `cliente-correo-gigachad` - executable or bundle name on disk.
* `Cliente de Correo` - display name shown in window titles and about app dialogs.
* `com.cliente.correo` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
* `Proyecto Redes` - the name of the company.
